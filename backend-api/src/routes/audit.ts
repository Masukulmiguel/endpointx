...
router.get('/', authenticate, requirePermission('logs.view'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const countResult = query('SELECT COUNT(*) as total FROM audit_logs', []);
    const total = countResult.rows[0]?.total || 0;
    const result = query('SELECT * FROM audit_logs ORDER BY created_at DESC LIMIT ? OFFSET ?', [limit, offset]);
...
router.get('/stats', authenticate, requirePermission('logs.view'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const byAction = query('SELECT action, COUNT(*) as count FROM audit_logs GROUP BY action ORDER BY count DESC', []);
    const recentCount = query("SELECT COUNT(*) as count FROM audit_logs WHERE created_at > datetime('now', '-24 hours')", []);
...
router.get('/export', authenticate, requirePermission('logs.export'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const result = query('SELECT * FROM audit_logs ORDER BY created_at DESC LIMIT 1000', []);
...
router.get('/:id', authenticate, requirePermission('logs.view'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const result = query('SELECT * FROM audit_logs WHERE id = ?', [req.params.id]);
