...
router.get('/events', authenticate, requirePermission('security.view'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const countResult = query('SELECT COUNT(*) as total FROM security_events', []);
    const total = countResult.rows[0]?.total || 0;

    const result = query('SELECT se.*, d.hostname FROM security_events se LEFT JOIN devices d ON se.device_id = d.id ORDER BY se.created_at DESC LIMIT ? OFFSET ?', [limit, offset]);
...
router.get('/events/stats', authenticate, requirePermission('security.view'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const total = query('SELECT COUNT(*) as count FROM security_events', []);
    const resolved = query('SELECT COUNT(*) as count FROM security_events WHERE is_resolved = 1', []);
    const unresolved = query('SELECT COUNT(*) as count FROM security_events WHERE is_resolved = 0', []);
    const bySeverity = query('SELECT severity, COUNT(*) as count FROM security_events GROUP BY severity', []);
    const byType = query('SELECT event_type, COUNT(*) as count FROM security_events GROUP BY event_type', []);
...
router.get('/events/:id', authenticate, requirePermission('security.view'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const result = query('SELECT * FROM security_events WHERE id = ?', [req.params.id]);
...
router.post('/events/:id/resolve', authenticate, requirePermission('security.manage'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    query("UPDATE security_events SET is_resolved = 1, resolved_by = ?, resolved_at = datetime('now') WHERE id = ?", [req.user?.id, req.params.id]);
