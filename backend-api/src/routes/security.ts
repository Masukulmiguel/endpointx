...
router.get('/events', authenticate, requirePermission('security.view'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await query('SELECT se.*, d.hostname FROM security_events se LEFT JOIN devices d ON se.device_id = d.id ORDER BY se.created_at DESC LIMIT $1 OFFSET $2', [limit, offset]);
...
router.get('/events/:id', authenticate, requirePermission('security.view'), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const result = await query('SELECT * FROM security_events WHERE id = $1', [req.params.id]);
